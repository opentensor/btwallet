    ...
    match env::var(&env_var_name) {
        Ok(encrypted_password_base64) => {
            ...
            let encrypted_password = general_purpose::STANDARD
                ...
                .map_err(|_| KeyFileError::EncryptionError("Invalid Base64".to_string()))?;
            // decrypting
...
// Decrypting password
fn decrypt_password(data: &Vec<u8>, key: String) -> String {
...
impl Keyfile {
    ...
    pub fn save_password_to_env(&self, password: Option<String>) -> Result<String, KeyFileError> {
        ...
        let encrypted_password = encrypt_password(env_var_name.clone(), password);
        let encrypted_password_base64 = general_purpose::STANDARD.encode(&encrypted_password);

        // saving password
        env::set_var(env_var_name.clone(), &encrypted_password_base64);
        Ok(encrypted_password_base64)
    }
